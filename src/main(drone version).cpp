...
int main(int argc, char** argv){
  ...
  //image_in2 : zed_mini로부터 실시간으로 들어오는 영상
    ...
    makeBirdEyeView(image_in1,image_bird2,0,0,K);
    ...
    message_filters::Subscriber<sensor_msgs::Image> image1_sub(nh,"/zedm/zed_node/rgb/image_rect_color",10);
    message_filters::Subscriber<sensor_msgs::Imu> rpy1_sub(nh,"/zedm/zed_node/imu/data",10);
    ...
    message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(500), image1_sub, rpy1_sub, pose1_sub);
    sync.registerCallback(boost::bind(image1_cb,_1,_2,_3));
...
EulerAngles ToEulerAngles(Quaternion q) {
    ...
    angles.roll = std::atan2(sinr_cosp, cosr_cosp)*(180/PI);

    // pitch (y-axis rotation)
    double sinp = 2 * (q.w * q.y - q.z * q.x);
    if (std::abs(sinp) >= 1)
        angles.pitch = std::copysign(M_PI / 2, sinp)*(180/PI); // use 90 degrees if out of range
    else
        angles.pitch = std::asin(sinp)*(180/PI);

    // yaw (z-axis rotation)
    double siny_cosp = 2 * (q.w * q.z + q.x * q.y);
    double cosy_cosp = 1 - 2 * (q.y * q.y + q.z * q.z);
    angles.yaw = std::atan2(siny_cosp, cosy_cosp)*(180/PI);

    return angles;
}


void image1_cb(const sensor_msgs::Image::ConstPtr& image,const sensor_msgs::Imu::ConstPtr& rpy,const geometry_msgs::PoseStamped::ConstPtr& pose){
  ...
  double altitude = pose->pose.position.z;
  ...
  if(image_in2.empty()){
    ...
    findRelativePose(image_bird2,image_bird2,K,altitude);
  }
...
int findRelativePose(Mat& in1, Mat& in2, Mat K,double altitude){
  //특징점 매칭//
  ...
  std::cout << "t = " << std::endl;
  for (auto t_ : Ts) {
    //double scale = altitude/6.0; 임시로 제거
    double scale = 0.71;
